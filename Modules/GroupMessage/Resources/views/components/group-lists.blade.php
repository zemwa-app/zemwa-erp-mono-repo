...
<div class="card rounded-0 border-top-0 border-left-0 border-right-0 user_list_box position-relative" id="user-no-{{ $message->id }}">
    ...
    <div class="dropdown position-absolute" style="top: 10px; right: 10px; padding-top: 25px;">
        <a href="javascript:;" class="text-dark-grey" id="dropdownMenuButton-group-{{ $message->id }}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        ...
        <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0" aria-labelledby="dropdownMenuButton-group-{{ $message->id }}" tabindex="0">
            <a class="dropdown-item view-group" href="javascript:;" data-group-id="{{ $message->id }}">@lang('app.view')</a>
            @if (user()->id == $message->owner_id || user()->isAdmin(user()->id))
                <a class="dropdown-item edit-group" href="javascript:;" data-group-id="{{ $message->id }}">@lang('app.edit')</a>
                <a class="dropdown-item delete-group" href="javascript:;" data-group-id="{{ $message->id }}">@lang('app.delete')</a>
            @endif
        </div>
    </div>
</div>
