@if (count($onboardingTasks) > 0)
    ...
    @php
        $showBoarding = false;
        $manageOnboardingPermission = user()->permission('manage_employee_onboarding');

        // Improved logic for determining when to show onboarding
        if ($employee->employeeDetail) {
            ...
            // Show onboarding if:
            // 1. Employee is new (onboarding_status = 'new')
            // 2. Onboarding is not completed (onboard_completed = 0)
            // 3. Admin viewing profile page
            // 4. User has manage permission
            if ($onboardingStatus === 'new' || $onboardCompleted === 0 || 
                (in_array('admin', user_roles()) && $type === 'profile') || 
                $manageOnboardingPermission === 'all') {
                $showBoarding = true;
            }
        }

        // Count tasks for progress tracking
        $totalOnboardTasks = count($onboardingTasks);
        $onBoardCompleteTasks = 0;
        
        if ($employee->employeeDetail) {
            $onBoardCompleteTasks = \Modules\Onboarding\Entities\OnboardingCompletedTask::where('employee_id', $employee->id)
                ->where('type', 'onboard')
    ...
    @if ($showBoarding && $totalOnboardTasks > 0)

        @php
            $action = '';

            // Calculate pending counts
            $pendingEmployeeOnboard = \Modules\Onboarding\Entities\OnboardingCompletedTask::where('employee_id', $employee->id)
                ->where('type', 'onboard')
                ->where('status', 'pending')
                ->whereHas('onboardingTask', function($q){ $q->where('task_for','employee'); })
                ->count();

            $pendingAnyOnboard = \Modules\Onboarding\Entities\OnboardingCompletedTask::where('employee_id', $employee->id)
                ->where('type', 'onboard')
                ->where('status', 'pending')
                ->count();

            // Show action buttons only if onboarding is in progress
            if ($employee->employeeDetail && $employee->employeeDetail->onboard_completed == 0) {
                $showMarkAll = in_array('admin', user_roles()) ? ($pendingAnyOnboard > 0) : ($pendingEmployeeOnboard > 0);

                if ($showMarkAll) {
                    $action = '<button type="button" class="btn btn-sm btn-primary d-inline-flex align-items-center mr-2" id="completeAllOnBoarding"> <i class="fa fa-check-circle mr-1"></i> ' . __('onboarding::messages.markAllAsComplete') . ' </button> ';
                }
            }
            ...
            if ($manageOnboardingPermission === 'all' || in_array('admin', user_roles())) {
                $action .= '<button type="button" class="btn btn-sm btn-danger d-inline-flex align-items-center" id="closeOnBoarding"> <i class="fa fa-undo mr-1"></i> ' . __('onboarding::messages.cancelOnBoarding') . ' </button>';
            }
        @endphp
        <x-cards.data class="mb-4 rounded shadow-sm break-inside-avoid dashboard-widget" :title="__('onboarding::clan.menu.onboardingTask')" padding="false"
            ...
            <div class="px-3">
                @foreach ($onboardingTasks as $task)
                    @php
                        ...
                        if ($manageOnboardingPermission === 'all') {
                            $hideTask = false;
                        }
                                                ...
                                                @elseif ($manageOnboardingPermission === 'all' || in_array('admin', user_roles()) || (!$isDisabled && $task->task_for == 'employee'))
                                                    @if ($isPending)
                                                        ...
                                                        @if ($task->completedTask->employee_id == user()->id)
                                                            <a href="javascript:;" 
                                                                class="action-button complete-action complete-onboard-task"
                                                    ...
                                                    @elseif ($isSubmitted)
                                                        ...
                                                        @if ($manageOnboardingPermission === 'all' || in_array('admin', user_roles()))
                                                            <a href="javascript:;" 
                                                                class="action-button approve-action approve-onboard-task"
                                                                data-task-id="{{ $task->completedTask->id }}">
                                                                <i class="fa fa-check mr-1"></i>
                                                                Approve
                                                            </a>
                                                            <a href="javascript:;" 
                                                                class="action-button reject-action reject-onboard-task"
                                                                data-task-id="{{ $task->completedTask->id }}">
                                                    ...
                                                    @elseif ($isApproved)
                                                        ...
                                                        @if ($manageOnboardingPermission === 'all' || in_array('admin', user_roles()))
                                                            <a href="javascript:;" 
                                                                class="action-button cancel-action cancel-onboard-task"
                                                                data-task-id="{{ $task->completedTask->id }}">
                                ...
                                <div class="task-meta text-muted small">
                                    @if ($task->completedTask && $task->completedTask['user_id'])
                                        ...
                                        @elseif ($task->completedTask['updated_at'])
                                            ...
                                            <strong>{{ \Carbon\Carbon::parse($task->completedTask['updated_at'])->translatedFormat(company()->date_format) }}</strong> <i class="fa fa-times-circle ms-2"></i>
                                            @lang('app.pending')
                                        @endif
                                    ...
                                    @if ($task->task_for == 'company')
                                        <i class="fa fa-building ms-2"></i> Company Task
                                    @elseif ($task->task_for == 'employee')
                                        <i class="fa fa-user ms-2"></i> Employee Task
                                    @else
                                        <i class="fa fa-list ms-2"></i> Company Task
                                    @endif
                                </div>
                            </div>
                        </div>
    ...
    @push('scripts')
        <script>
            $(document).ready(function() {
                ...
                // Reject task
                $('.reject-onboard-task').on('click', function() {
                    var taskId = $(this).data('task-id');
                ...
                // Cancel task
                $('.cancel-onboard-task').on('click', function() {
                    var taskId = $(this).data('task-id');
                ...
                $('.complete-onboard-task').on('click', function() {
                    var taskId = $(this).data('task-id');
                ...
                $(document).on('click', '#completeAllOnBoarding', function() {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            var userRoles = @json(user_roles());
                            var isAdmin = @json(in_array('admin', user_roles()));
