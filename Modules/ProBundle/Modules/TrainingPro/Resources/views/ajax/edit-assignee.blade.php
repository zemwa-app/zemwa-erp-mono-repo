							...
							<div class="col-sm-12 col-md-4 col-lg-4 col-xl-4">
								...
								<x-forms.select fieldId="department_id" :fieldLabel="__('app.department')"
									...
									@foreach ($departments as $team)
										<option value="{{ $team->id }}" {{ $team->id == $assignee->department_id ? 'selected' : '' }}>{{ $team->team_name }}</option>
									@endforeach
								</x-forms.select>
							</div>
								...
								<x-forms.input-group>
									<select class="form-control select-picker" name="designation"
										...
										@foreach ($designations as $designation)
											<option value="{{ $designation->id }}" {{ $designation->id == $assignee->designation_id ? 'selected' : '' }}>{{ $designation->name }}</option>
										@endforeach
									</select>
								</x-forms.input-group>
							</div>
									...
									<x-forms.input-group>
										<select class="form-control select-picker" name="user_id"
											...
											@foreach($employees as $employee)
													<option  data-content="<div class='d-inline-block mr-1'><img class='taskEmployeeImg rounded-circle' src='{{ $employee->image_url }}' ></div> {{ $employee->name }}" value="{{ $employee->id }}" {{ $employee->id == $assignee->user_id ? 'selected' : '' }}>
														{{ $employee->name }}
													</option>
											@endforeach
										</select>
								...
								<x-forms.input-group>
									<select class="form-control select-picker" name="assignment_category"
										...
										@foreach ($categories as $category)
											<option value="{{ $category->id }}" {{ $category->id == $assignee->category_id ? 'selected' : '' }}>{{ $category->name }}</option>
										@endforeach
									</select>
								</x-forms.input-group>
							</div>
								...
								<x-forms.input-group>
									<select class="form-control select-picker" name="assignment_programme"
										...
										@foreach ($programmes as $programme)
											<option value="{{ $programme->id }}" {{ $programme->id == $assignee->programme_id ? 'selected' : '' }}>{{ $programme->name }}</option>
										@endforeach
									</select>
								</x-forms.input-group>
							</div>
							...
							<div class="col-md-2 col-lg-2 col-xl-2">
								<x-forms.input-group class="d-flex align-items-end">
									<x-forms.text fieldId="assignee_order" fieldLabel="Order"
												  fieldName="assignee_order" fieldValue="{{ $assignee->order }}" fieldPlaceholder="e.g. 0">
									</x-forms.text>
								</x-forms.input-group>
							</div>
						</div>
					...
					<div class="col-4 d-flex align-items-center justify-content-start">
						<div class="custom-control custom-switch d-flex align-items-center justify-content-center">
							...
							<div class="custom-control custom-switch">
								<input id="assignee_status" class="custom-control-input" type="checkbox" name="assignee_status" value="{{ $assignee->is_enabled }}" {{ $assignee->is_enabled == 1 ? 'checked' : '' }}>
...
<script>
	$(document).ready(function() {
		...
		$('#department_id').change(function() {
			...
			$.easyAjax({
				url: url,
				container: '#editAssigneeForm',
				type: "GET",
				blockUI: true,
				//data: $('#editAssigneeForm').serialize(),
				dataType: "json",
				headers: {
		...
		$('#save-assignee').click(function() {
			...
			var formData = $('#editAssigneeForm').serialize();
