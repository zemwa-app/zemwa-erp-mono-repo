...
@section('content')
		...
		<div class="row p-3 justify-content-around">
			...
			<div class="col-sm-6 col-md-3 rounded border border-grey bg-white py-4">
				<div class="text-center text-danger">
					...
					<h2 class="h5 mb-4">{{ $topics }} Topics</h2>
				</div>
			</div>
			<div class="col-sm-6 col-md-3 rounded border border-grey bg-white py-4">
				<div class="text-center text-danger">
					...
					<h2 class="h5 mb-4">{{ $programmes }} Programmes</h2>
				</div>
			</div>
			<div class="col-sm-6 col-md-3 rounded border border-grey bg-white py-4">
				<div class="text-center text-danger">
					...
					<h2 class="h5 mb-4">{{ $categories }} Categories</h2>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="container-fluid">
		<div class="row p-3">
					...
					@if(count($recentAssessments) != 0)
						<table class="table">
							...
							<tr>
								...
								<th scope="col" class="col-sm-1 align-middle">Topic</th>
								<th scope="col" class="col-sm-2 align-middle">Programme</th>
							...
							@foreach($recentAssessments as $index => $recentAssessment)
								<tr>
									<td>{{ $index + 1 }}</td>
									<td>{{ $recentAssessment->name ?: 'Not defined' }}</td>
									<td>{{ $recentAssessment->topic->name ?: 'Not defined' }}</td>
									<td>{{ $recentAssessment->topic->programmes->name ?: 'Not defined' }}</td>
									<td>{{ $recentAssessment->topic->programmes->category->name ?: 'Not defined' }}</td>
			...
			<div class="col-sm-12 col-md-6 rounded border border-grey bg-white py-4">
				<div class="col-sm-12 mb-2 pb-2 border-bottom-grey"><h4>Employee Assessment Results:</h4><small>(Submitted this month)</small></div>
				<div class="col table-responsive">
					@if(count($recentResults) != 0)
						<table class="table">
							...
							<tr>
								...
								<th scope="col" class="col-sm-3 align-middle">Topic</th>
							...
							@foreach($recentResults as $index => $recentResult)
								<tr>
									<td>{{ $index + 1 }}</td>
									<td>{{ $recentResult->username ?: 'Not defined' }}</td>
									<td>{{ $recentResult->assessment ?: 'Not defined' }}</td>
									<td>{{ $recentResult->topic ?: 'Not defined' }}</td>
									<td>{{ $recentResult->score ?: 'Not defined' }}</td>
									<td>{{ $recentResult->updated_at ? \Carbon\Carbon::parse($recentResult->updated_at)->format('d M.') : 'Not defined' }}
									</td>
								</tr>
							@endforeach
							</tbody>
