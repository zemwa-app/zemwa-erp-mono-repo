...
class ProjectRoadmapDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->editColumn('deadline', function ($row) {
            ...
            if ($row->deadline->endOfDay()->isPast()) {
                return '<span class="text-danger">' . $row->deadline->translatedFormat($this->company->date_format) . '</span>';
            }
            elseif ($row->deadline->isToday()) {
                return '<span class="text-success">' . __('app.today') . '</span>';
            }

            return '<span>' . $row->deadline->translatedFormat($this->company->date_format) . '</span>';
        });
        $datatables->addColumn(
        ...
        $datatables->rawColumns(array_merge(['project_name', 'completion_percent', 'members', 'status', 'client_id', 'project_short_code', 'action', 'deadline'], $customFieldColumns));
