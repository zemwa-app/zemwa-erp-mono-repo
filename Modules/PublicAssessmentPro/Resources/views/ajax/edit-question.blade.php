...
<div class="row">
	<div class="col-sm-12">
		<x-form id="editQaAnsForm">
			<div class="add-client bg-white rounded">
				<h5 class="mb-0 p-20 font-weight-normal border-bottom-grey">@lang('publicassessmentpro::app.header.qaEditHeader') "{{ $paAssessment->assessment_name }}"</h5>
                <div class="row p-20">
                    <div class="col-lg-12">
                        <div class="row">
                    ...
                    @if ($paAssessment->assessment_type==0)
                        <div class="row">
							<div class="col-sm-12 col-md-10 col-lg-10 col-xl-10">
								<input type="hidden" name="quesId" value="{{ $paQuestion->id }}">
								...
								<x-forms.text fieldId="assesQa" :fieldLabel="__('publicassessmentpro::app.form.assesQuestion')"
											  fieldName="assesQa" fieldValue="{{ $paQuestion->question }}" fieldRequired="true" :fieldPlaceholder="__('publicassessmentpro::app.form.placeholderQuestion')">
								</x-forms.text>
							</div>

							<div class="col-sm-12 col-md-2 col-lg-2 col-xl-2">
								<x-forms.number fieldId="assesMark" :fieldLabel="__('publicassessmentpro::app.form.assesMark')"
												fieldName="assesMark" fieldValue="{{ $paQuestion->score }}" fieldRequired="true" :fieldPlaceholder="__('publicassessmentpro::app.form.placeholderMark')">
								</x-forms.number>
							</div>
							...
							<div class="col-lg-6 col-md-6 col-xl-3">
								<x-forms.input-group class="d-flex align-items-end">
									<x-forms.text fieldId="assesAnsOne" :fieldLabel="__('publicassessmentpro::app.form.assesAnswerOne')"
												  fieldName="assesAnsOne" fieldValue="{{ $paQuestion->answers[0]->answer }}" fieldRequired="true" :fieldPlaceholder="__('publicassessmentpro::app.form.placeholderAnsOne')">
									</x-forms.text>
									<x-slot name="append">
							...
							<div class="col-md-6 col-lg-4 col-xl-3">
								<x-forms.input-group class="d-flex align-items-end">
									<x-forms.text fieldId="assesAnsTwo" :fieldLabel="__('publicassessmentpro::app.form.assesAnswerTwo')"
												  fieldName="assesAnsTwo" fieldValue="{{ $paQuestion->answers[1]->answer }}" fieldRequired="true" :fieldPlaceholder="__('publicassessmentpro::app.form.placeholderAnsTwo')">
									</x-forms.text>
									<x-slot name="append">
										<label class="btn btn-outline-light form-control btnRightAns" style="margin-bottom:16px; padding-top:10px; padding-bottom:11px">
							...
							<div class="col-md-6 col-lg-4 col-xl-3">
								<x-forms.input-group class="d-flex align-items-end">
									<x-forms.text fieldId="assesAnsThree" :fieldLabel="__('publicassessmentpro::app.form.assesAnswerThree')"
												  fieldName="assesAnsThree" fieldValue="{{ $paQuestion->answers[2]->answer }}" :fieldPlaceholder="__('publicassessmentpro::app.form.placeholderAnsThree')">
									</x-forms.text>
									<x-slot name="append">
										<label class="btn btn-outline-light form-control btnRightAns" style="margin-bottom:16px; padding-top:10px; padding-bottom:11px">
							...
							<div class="col-md-6 col-lg-4 col-xl-3">
								<x-forms.input-group class="d-flex align-items-end">
									<x-forms.text fieldId="assesAnsFour" :fieldLabel="__('publicassessmentpro::app.form.assesAnswerFour')"
												  fieldName="assesAnsFour" fieldValue="{{ $paQuestion->answers[3]->answer }}" :fieldPlaceholder="__('publicassessmentpro::app.form.placeholderAnsFour')">
									</x-forms.text>
									<x-slot name="append">
										<label class="btn btn-outline-light form-control btnRightAns" style="margin-bottom:16px; padding-top:10px; padding-bottom:11px">
...
<script>
	$(document).ready(function() {
        ...
        function refreshSelectPicker(selectId) {
            var selectedCat = "{{ $paQuestion->quest_cat_id }}";
            $('#' + selectId).val(selectedCat);
            $('#' + selectId).selectpicker('refresh');
           
        }

		var correctAnswer = "{{ $paQuestion->correct_answer }}";
		var radioButton = $('input[name="rightAns"][value="' + correctAnswer + '"]');

		if (radioButton.length !== 0) {
			radioButton.prop('checked', true);
			radioButton.siblings('[data-icon="check"]').css('opacity', 1);
		} else {
			console.error("Radio button with value " + correctAnswer + " not found.");
		}

		 $('#tpQsStatus').prop('checked', {{ $paQuestion->status }} == 1);
        ...
        var formData = $('#editQaAnsForm').serialize();
        ...
        //var url = '{{ route("publicassessmentpro.config.getQuesCategoryList", ['action' => 'placeholder']) }}'.replace('placeholder', 'edit');
        ...
        // called here on view category button click or on document load - check later
