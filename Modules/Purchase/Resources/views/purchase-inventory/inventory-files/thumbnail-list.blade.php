    ...
    @forelse($inventory->files as $file)
        ...
        @if ($viewPermission == 'all' || ($viewPermission == 'added' && $file->added_by == user()->id))
            <div class="col-md-4 col-lg-3 mt-2">
                <x-file-card :fileName="$file->filename" :dateAdded="$inventory->created_at->diffForHumans()">
                    @if ($file->icon == 'images')
                                ...
                                <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0"
                                    ...
                                    @if ($viewPermission == 'all' || ($viewPermission == 'added' && $file->added_by == user()->id))
                                        ...
                                        <a class="cursor-pointer d-block text-dark-grey f-13 py-3 px-3 "
                                            href="{{ route('inventory-files.download', $file->id) }}">@lang('app.download')</a>
                                    @endif

                                    @if ($deletePermission == 'all' || ($deletePermission == 'added' && $file->added_by == user()->id))
                                        <a class="cursor-pointer d-block text-dark-grey f-13 pb-3 px-3 delete-inventory-file"
