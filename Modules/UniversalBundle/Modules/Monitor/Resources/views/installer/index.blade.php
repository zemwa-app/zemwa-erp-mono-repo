            ...
            <div class="col-lg-12">
                ...
                @if ($can_manage && !App::environment('demo'))
                    <x-cards.data :title="__('monitor::app.manageInstallers')" class="mb-4">
                        ...
                        <x-form id="upload-installer-form" method="POST" action="{{ route('monitor.installer.upload') }}">
                            <div class="row">
                                <div class="col-md-6">
                                    <x-forms.text
                ...
                <x-cards.data :title="__('monitor::app.downloadAgentInstaller')">
                    <p class="f-14 text-dark-grey mb-4">
                        @lang('monitor::app.installerIntro')
                    </p>

                    <div class="mb-4">
                        @foreach ($platforms as $platform)
                            @if ($platform['available'])
                                <a href="{{ $platform['download_url'] }}"
                                    class="monitor-platform-link mb-3 is-available">
                                    <span class="d-flex align-items-center">
                                        @php
                                            $platformIconStyles = [
                                                'windows' => 'background:#eef2ff;color:#4f46e5;',
                                                'mac' => 'background:#f1f5f9;color:#334155;',
                                                'ubuntu' => 'background:#fff7ed;color:#ea580c;',
                                            ];
                                            $platformKey = $platform['key'] ?? '';
                                        @endphp
                                        <span class="monitor-platform-icon mr-3"
                                            style="{{ $platformIconStyles[$platformKey] ?? '' }}">
                                            <i class="{{ $platform['icon'] }}" aria-hidden="true"></i>
                                        </span>
                                        <span>
                                            <span class="d-block f-14 f-w-500 text-darkest-grey">@lang('monitor::app.download') {{ $platform['label'] }}</span>
                                            @if ($platform['size_label'])
                                                <span class="f-12 text-dark-grey">{{ $platform['size_label'] }}</span>
                                            @endif
                                        </span>
                                    </span>
                            ...
                            @else
                                <div class="monitor-platform-link mb-3 is-pending">
                                    <span class="d-flex align-items-center">
                                        <span class="monitor-platform-icon mr-3" style="background:#fffbeb;color:#d97706;">
                                            <i class="{{ $platform['icon'] }}" aria-hidden="true"></i>
                                        </span>
                                        <span>
                                            <span class="d-block f-14 f-w-500 text-warning">@lang('monitor::app.download') {{ $platform['label'] }}</span>
                                            <span class="f-12 text-warning">@lang('monitor::app.installerUnavailable')</span>
                                        </span>
                                    </span>
                    ...
                    <div class="card bg-white border border-grey rounded mb-4">
                        <div class="card-body p-20">
                            <h4 class="f-14 f-w-500 text-darkest-grey mb-3">@lang('monitor::app.installationInstructions')</h4>
                            <ol class="pl-4 f-14 text-dark-grey mb-0">
                                <li class="mb-2">@lang('monitor::app.installStep1')</li>
                                <li class="mb-2">@lang('monitor::app.installStep2')</li>
                                <li>@lang('monitor::app.installStep3')</li>
                            </ol>
                        </div>
                    </div>

                    @if ($version)
                        <p class="border-top-grey pt-4 text-center f-12 text-lightest mb-0">
                            @lang('monitor::app.agentVersionInfo', ['version' => $version, 'date' => $released_at_label])
                        </p>
                    @endif
                </x-cards.data>
            </div>
...
@push('scripts')
    <script>
        (function () {
            ...
            const uploadUrl = @json(route('monitor.installer.upload'));
            ...
            $('#upload-installer-btn').on('click', function (e) {
                ...
                const form = document.getElementById('upload-installer-form');
