<div class="modal-header">
    <h5 class="modal-title">@lang('onboarding::clan.menu.editOnboardingTask')</h5>
    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
</div>
<x-form id="editOnboarding" method="POST" class="ajax-form">
    @method('PUT')
    ...
    <div class="modal-footer">
        ...
        <x-forms.button-primary id="update-onboarding-setting" icon="check">@lang('app.update')</x-forms.button-primary>
    </div>
</x-form>

<script>
    $(document).ready(function() {
        // Function to toggle the visibility of the "Employee can see" checkbox
        function toggleEmployeeCheckbox() {
            if ($('#company').is(':checked')) {
                $('#employee-access').show();
                $('#task_for').val('company'); // Update the hidden input value
            } else {
                $('#employee-access').hide();
                $('#task_for').val('employee'); // Update the hidden input value
            }
        }

        // Call the toggle function when the page is loaded
        toggleEmployeeCheckbox();

        // Toggle the checkbox visibility when the radio buttons are clicked
        ...
        $('#employee-access-checkbox').change(function () {
            // Set employee_can_see value based on checkbox status
            if ($(this).is(':checked')) {
