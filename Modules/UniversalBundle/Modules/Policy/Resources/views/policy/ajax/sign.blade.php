    ...
    <x-form id="acceptEstimates">
        <div class="row">
            <div class="pl-4 col-md-12">
                <p>@lang('policy::messages.confirmPolicy')</p>
            </div>
            <div class="p-4 col-sm-12 bg-grey signature">
                <x-forms.label fieldId="sign-pad" fieldRequired="true" :fieldLabel="__('modules.estimates.signature')" />
                <div class="border-0 signature_wrap wrapper form-control">
                    <canvas id="sign-pad" class="rounded signature-pad" width=400 height=150></canvas>
                </div>
            </div>
            <div class="p-4 col-sm-12 d-none upload-img">
                <x-forms.file allowedFileExtensions="png jpg jpeg svg bmp" class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('modules.estimates.signature')"
                    fieldName="image" fieldId="sign_image" :popover="__('messages.fileFormat.ImageFile')" fieldRequired="true" accept="image/png, image/jpeg, image/svg+xml, image/bmp"/>
            </div>
            <div class="pl-4 mt-3 col-md-12">
...
<div class="modal-footer">
    ...
    <x-forms.button-primary id="save-sign" icon="check">@lang('policy::app.acknowledge')</x-forms.button-primary>
</div>
...
<script>
    ...
    $('#save-sign').click(function() {
        ...
        $(this).text('{{ __('policy::messages.processing') }}');
