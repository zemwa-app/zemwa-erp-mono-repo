...
@section('filter-section')
    <x-filters.filter-box>
        ...
        <x-filters.more-filter-box>
            ...
            <div class="more-filter-items">
                <label class="mb-12 f-14 text-dark-grey text-capitalize" for="usr">@lang('modules.employees.gender')</label>
                <div class="mb-4 select-filter">
                    <select class="form-control select-picker" name="gender" id="gender" data-live-search="true" data-size="8">
                        <option value="all">@lang('app.all')</option>
        ...
        <div class="d-grid d-lg-flex d-md-flex action-bar">
            <div id="table-actions" class="flex-grow-1 align-items-center mb-2 mb-lg-0 mb-md-0">
                @if ($addPermission == 'all')
                    <x-forms.link-primary :link="route('policy.create')" class="float-left mr-3 openRightModal" icon="plus">
                            @lang('app.add') @lang('policy::app.policy')
                    </x-forms.link-primary>
                @endif
            </div>

            <div class="btn-group mt-2 mt-lg-0 mt-md-0 ml-0 ml-lg-3 ml-md-3" role="group">
                <a href="{{ route('policy.index') }}" class="btn btn-secondary f-14 btn-active policies" data-toggle="tooltip"
                    data-original-title="@lang('policy::app.policies')"><i class="side-icon bi bi-list-ul"></i></a>

                @if ($archivePermission != 'none' || in_array('admin', user_roles()))
                    <a href="{{ route('policy.archive') }}" class="btn btn-secondary f-14" data-toggle="tooltip"
...
@push('scripts')
    ...
    <script>
        ...
        $('#policy-table').on('preXhr.dt', function(e, settings, data) {
        ...
        $('#search-text-field, #department, #designation, #employmentType, #signatureRequired, #gender').on('change keyup', function() {
        ...
        const showTable = () => {
            window.LaravelDataTables["policy-table"].draw(true);
        }
        ...
        $('body').on('click', '.archive-policy', function() {
            var id = $(this).data('policy-id');
            Swal.fire({
                title: "@lang('messages.sweetAlertTitle')",
                text: "@lang('policy::messages.archiveMessage')",
                icon: 'warning',
                showCancelButton: true,
                focusConfirm: false,
