<div class="modal-header">
    <h5 class="modal-title">@lang('app.add') @lang('recruit::modules.setting.field')</h5>
    ...
    <div class="portlet-body">
        <x-form id="createForm" method="POST" class="ajax-form">
            <div class="row">
                ...
                <div class="col-lg-6">
                    <x-forms.select fieldId="category_id" :fieldLabel="__('modules.tasks.category')" fieldName="category_id"
                                    search="true">
                                    @foreach ($customFieldGroups as $group)
                                        <option value="{{ $group->id }}"
                                                data-content="{{ $group->name }}"></option>
                                    @endforeach
                    </x-forms.select>
                </div>
...
<script>
    ...
    $('#save-custom-field').click(function () {
        $.easyAjax({
            url: "{{route('custom-field-settings.store')}}",
