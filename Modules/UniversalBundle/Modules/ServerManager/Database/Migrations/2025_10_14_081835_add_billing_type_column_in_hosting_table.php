...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        DB::table('server_hostings')->where('billing_cycle', 'monthly')->update([
            'billing_cycle_count' => 1,
            'billing_type' => 'month'
        ]);

        DB::table('server_hostings')->where('billing_cycle', 'quarterly')->update([
            'billing_cycle_count' => 3,
            'billing_type' => 'month'
        ]);

        DB::table('server_hostings')->where('billing_cycle', 'semi-annually')->update([
            'billing_cycle_count' => 6,
            'billing_type' => 'month'
        ]);

        DB::table('server_hostings')->where('billing_cycle', 'annually')->update([
            'billing_cycle_count' => 1,
            'billing_type' => 'year'
        ]);

        DB::table('server_hostings')->where('billing_cycle', 'biennially')->update([
