...
class InvoiceRecurringDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->editColumn(
                ...
                'next_invoice_date',
                function ($row) {
                    $color = RecurringInvoice::ROTATION_COLOR[$row->rotation] ?? 'primary';
                    $rotation = '<span class="px-1"><label class="badge badge-' . $color . '">' . e($row->rotation_label) . '</label></span>';
    ...
    public function query(RecurringInvoice $model)
    {
        ...
        $model = $model->with(['project' => function ($q) {
            ...
            ->select('invoice_recurring.id', 'invoice_recurring.project_id', 'invoice_recurring.client_id', 'invoice_recurring.currency_id', 'invoice_recurring.billing_cycle', 'invoice_recurring.total', 'invoice_recurring.status', 'invoice_recurring.issue_date', 'invoice_recurring.show_shipping_address', 'invoice_recurring.client_can_stop', 'invoice_recurring.next_invoice_date', 'invoice_recurring.rotation', 'invoice_recurring.billing_interval', 'invoice_recurring.billing_unit');
