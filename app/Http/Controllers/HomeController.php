...
class HomeController extends Controller
{
     ...
     * @return \Illuminate\Http\Response
     */
    public function leadForm($hash, $slug = null)
    {
        ...
        $this->company = Company::where('hash', $hash)->firstOrFail();
        ...
        $formQuery = LeadForm::where('company_id', $this->company->id)->where('status', 'active');

        if ($slug) {
            $this->leadForm = $formQuery->where('slug', $slug)->firstOrFail();
        } else {
            $this->leadForm = $formQuery->where('is_default', 1)->firstOrFail();
        }

        $this->category = $this->leadForm->category_id
            ? LeadCategory::select('id')->find($this->leadForm->category_id)
            : LeadCategory::select('id')->where('company_id', $this->company->id)->where('is_default', 1)->first();

        $this->leadFormFields = LeadCustomForm::with('customField')
            ->where('status', 'active')
            ->where('lead_form_id', $this->leadForm->id)
    ...
    public function leadStore(StorePublicLead $request)
    {
        ...
        $leadForm = LeadForm::where('company_id', $company->id)
            ->where('id', $request->lead_form_id)
            ->where('status', 'active')
            ->firstOrFail();

        $leadPipeline = $leadForm->lead_pipeline_id
            ? LeadPipeline::find($leadForm->lead_pipeline_id)
            : LeadPipeline::where('default', '1')->where('company_id', $company->id)->first();

        $leadStage = $leadForm->pipeline_stage_id
            ? PipelineStage::find($leadForm->pipeline_stage_id)
            : PipelineStage::where('default', '1')->where('lead_pipeline_id', $leadPipeline->id)->where('company_id', $company->id)->first();

        $categoryId = $leadForm->category_id ?? $request->category_id ?? null;

        if (is_null($categoryId)) {
            $defaultCategory = LeadCategory::where('company_id', $company->id)->where('is_default', 1)->first();
        ...
        $lead->source_id = $request->source ?? $leadForm->lead_source_id;
    ...
    public function domPdfObjectProposalDownload($id)
    {
        ...
        $pdf->loadHTML(pdfFixArabic(view('proposals.pdf.' . $this->invoiceSetting->template, $this->data)->render()));
