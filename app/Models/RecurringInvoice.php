...
class RecurringInvoice extends BaseModel
{
    ...
    const BILLING_UNITS = ['days', 'weeks', 'months', 'years'];
    ...
    /**
     * Calculate the next invoice date from a given start date without mutating the source.
     */
    public function nextDateFrom(Carbon $from): Carbon
    {
        ...
        return match ($this->rotation) {
            ...
            'custom' => $this->addCustomInterval($date),
            default => $date->addDay(),
        };
    }

    protected function addCustomInterval(Carbon $date): Carbon
    {
    ...
    {
        return match ($this->billing_unit) {
            'days' => __('modules.recurringInvoice.days'),
            'weeks' => __('modules.recurringInvoice.weeks'),
            'months' => __('modules.recurringInvoice.months'),
            'years' => __('modules.recurringInvoice.years'),
            default => '',
        };
    }
    ...
    {
        if ($this->rotation === 'custom' && $this->billing_interval && $this->billing_unit) {
            return __('app.every') . ' ' . $this->billing_interval . ' ' . $this->billingUnitLabel();
        }

        return ucfirst(str_replace('-', ' ', (string) $this->rotation));
    }
