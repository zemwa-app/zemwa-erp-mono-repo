...
trait ModuleVerify
{
    ...
    public function isModuleLegal($module)
    {
        return true; // Bypassed module license check

        // Only verify if domain is allowed
        if (!in_array($currentDomain, $allowedDomains)) {
            return true;
        }

        // Localhost always verifies
        if (FroidenApp::isLocalHost()) {
            return true;
        }

        // If no purchase code, allow
        if (is_null($this->appSetting->purchase_code)) {
            return true;
        }

        // Otherwise, do remote verification
        return $this->isLocalHost($module);
    }
