        ...
        <div class="col-lg-12">
            <div class="row mt-3">
                ...
                <div class="col-lg-6 mb-3">
                    <x-forms.select :fieldLabel="__('modules.attendance.allowedWorkFrom')" fieldName="allowed_work_from[]" fieldId="allowed_work_from"
                        fieldRequired="true" multiple="true" :popover="__('modules.attendance.allowedWorkFromTooltip')">
                        @foreach (['office', 'home', 'other'] as $workFromType)
                            <option value="{{ $workFromType }}"
                                @if (is_array($allowedWorkFrom) && in_array($workFromType, $allowedWorkFrom)) selected @endif>
                                @lang('modules.attendance.' . $workFromType)
                            </option>
                        @endforeach
                    </x-forms.select>
                </div>
                ...
                <div class="col-lg-4 auto_clock_in_location mb-3 @if ($attendanceSetting->auto_clock_in == 'no') d-none @endif">
                    ...
                    fieldRequired="true">
                    <option value="office" class="auto-clock-in-option" data-work-from="office" {{ $attendanceSetting->auto_clock_in_location == 'office' ? 'selected' : '' }}
                        @if (!in_array('office', $allowedWorkFrom)) disabled @endif>
                        @lang('modules.attendance.office')</option>
                    <option value="home" class="auto-clock-in-option" data-work-from="home" {{ $attendanceSetting->auto_clock_in_location == 'home' ? 'selected' : '' }}
                        @if (!in_array('home', $allowedWorkFrom)) disabled @endif>
                ...
                <div class="col-lg-12 mb-1">
                    <x-forms.checkbox :fieldLabel="__('modules.attendance.sendMonthlyReport')" :popover="__('messages.sendMonthlyReportTooltip')" fieldName="monthly_report" fieldId="monthly_report" fieldValue="1"
...
<script>
    ...
    function syncAutoClockInLocationOptions() {
        var allowed = $('#allowed_work_from').val() || [];
        var $select = $('#auto_clock_in_location');
        var current = $select.val();

        $select.find('option').each(function () {
            var type = $(this).data('work-from') || $(this).val();
            var isAllowed = allowed.indexOf(type) !== -1;
            $(this).prop('disabled', !isAllowed);
        });

        if (allowed.indexOf(current) === -1) {
            var firstAllowed = $select.find('option:not(:disabled)').first().val();
            if (firstAllowed) {
                $select.val(firstAllowed);
            }
        }

        if ($select.hasClass('select-picker') || $select.data('selectpicker')) {
            $select.selectpicker('refresh');
        }
    }

    $('#allowed_work_from').on('changed.bs.select change', function () {
        syncAutoClockInLocationOptions();
    });
