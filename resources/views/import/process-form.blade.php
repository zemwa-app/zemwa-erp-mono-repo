        ...
        <div class="add-{{ $importClassName }} bg-white rounded">
            <h4 class="mb-0 p-20 f-21 font-weight-normal  border-bottom-grey">
                ...
                <div class="col-md-12 import-table">
                    ...
                    <div class="row">
                        @php
                            $firstSampleRow = $importSample[0] ?? (is_array($importSample) && count($importSample) ? $importSample[array_key_first($importSample)] : []);
                        @endphp
                        @forelse ($firstSampleRow as $key => $item)
                            @php
                                $headingKey = !empty($heading) ? ($heading[$key] ?? null) : null;
                                $isMatched = $headingKey ? (bool) collect($columns)->where('id', $headingKey)->first() : false;
                            @endphp
                            <div class="col-md-3 importBox  border-grey {{ !empty($heading) ? ($isMatched ? 'matched' : 'unmatched') : 'unmatched' }}"
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div id="selectOptionList_{{ $key }}">
                                                    <select class="form-control select-picker mb-2" id="columnName_{{ $key }}" name="columns[{{ $key }}]">
                                                        ...
                                                        @if (!empty($heading) && ($heading[$key] ?? null) && collect($columns)->where('id', $heading[$key])->first())
                                                            @foreach($columns as $selectKey => $selectColumn)
                                                            <option value="{{ $selectColumn['id'] }}" {{ (($heading[$key] ?? null)==$selectColumn['id']) ? 'selected' : '' }}>{{$selectColumn['name'] }}
                                                            </option>
                                                            @endforeach
                                                        @endif
                                                    </select>
                                    ...
                                    <div class="row columnDescriptionBox" id="columnDescriptionBox_{{ $key }}">
                                        <div class="col-sm-12">
                                            <p id="columnDescriptionBoxText_{{ $key }}">
                                                @if (!empty($heading))
                                                    @if(($heading[$key] ?? null) && collect($columns)->where('id', $heading[$key])->first())
                                                        {{ collect($columns)->where('id', $heading[$key])->first()['name'] }}
                                                    @else
