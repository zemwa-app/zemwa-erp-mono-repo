                ...
                <tr id="archivetype-{{ $leaveType->id }}">
                    ...
                    <td>
                        @if ($leaveType->isUnlimited())
                            @lang('app.unlimitedLeaveType')
                        @else
                            {{ ucfirst($leaveType->leavetype) }}
                        @endif
                    </td>
                    <td> {{ $leaveType->isUnlimited() ? '--' : $leaveType->no_of_leaves }}</td>
                    <td> {{ ($leaveType->isUnlimited() || $leaveType->monthly_limit <= 0) ? '--' : $leaveType->monthly_limit }}</td>
                    <td>
                        @if ($leaveType->paid == 1)
...
<script>
    ...
    $('body').on('click', '.restore-category', function() {
        ...
        Swal.fire({
            title: "@lang('messages.sweetAlertTitle')",
            text: "@lang('messages.unArchiveMessageLeave')",
            icon: 'warning',
            showCancelButton: true,
            focusConfirm: false,
