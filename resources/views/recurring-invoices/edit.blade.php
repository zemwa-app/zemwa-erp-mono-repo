        ...
        <div class="bg-white rounded b-shadow-4 create-inv">
            ...
            <!-- INVOICE NUMBER, DATE, DUE DATE, FREQUENCY START -->
                ...
                <div class="row px-lg-4 px-md-4 px-3 py-3">
                    ...
                    @php $payfastActive = isset($paymentGateway) && $paymentGateway->payfast_status === 'active'; @endphp
                    <div class="col-lg-3">
                        <x-forms.checkbox class="mr-0 mr-lg-2 mr-md-2"
                                               :fieldLabel="__('modules.recurringInvoice.enableAutoPay')"
                                               fieldName="enable_auto_pay"
                                               fieldId="enable_auto_pay"
                                               fieldValue="1"
                                               :checked="$invoice->enable_auto_pay == 1"
                                               :fieldPermission="!$payfastActive"/>
                        @if(!$payfastActive)
                            <small class="text-muted f-12">@lang('modules.recurringInvoice.payfastRequired')</small>
                        @endif
                    </div>
                ...
                <div class="row px-lg-4 px-md-4 px-3 pt-3">
                            ...
                            <div class="col-md-4 mt-4">
                                <div class="form-group c-inv-select mb-4">
                                    ...
                                    <select class="form-control select-picker" data-live-search="true" data-size="8"
                                        ...
                                        <option value="custom" @if($invoice->rotation == 'custom') selected @endif>@lang('app.custom')</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-4 mt-4 {{ $invoice->rotation == 'custom' ? '' : 'd-none' }}" id="custom_interval_fields">
                                <x-forms.label fieldId="billing_interval" :fieldLabel="__('modules.recurringInvoice.customInterval')"
                                            fieldRequired="true">
                                </x-forms.label>
                                <div class="form-group">
                                    <div class="input-group">
                                        <input type="number" min="1" class="form-control height-35 f-15" name="billing_interval"
                                               id="billing_interval" value="{{ $invoice->billing_interval ?? 1 }}" {{$recurringInvoice}}>
                                        <div class="input-group-append" style="min-width: 120px;">
                                            <select class="form-control select-picker" name="billing_unit" id="billing_unit" {{$recurringInvoice}}>
                                                <option value="days" @if(($invoice->billing_unit ?? '') == 'days') selected @endif>@lang('modules.recurringInvoice.days')</option>
                                                <option value="weeks" @if(($invoice->billing_unit ?? '') == 'weeks') selected @endif>@lang('modules.recurringInvoice.weeks')</option>
                                                <option value="months" @if(($invoice->billing_unit ?? 'months') == 'months') selected @endif>@lang('modules.recurringInvoice.months')</option>
                                                <option value="years" @if(($invoice->billing_unit ?? '') == 'years') selected @endif>@lang('modules.recurringInvoice.years')</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                    ...
                    @php
                        ...
                        case 'annually':
                            ...
                            break;
                        case 'custom':
                            $rotationType = ($invoice->billing_interval ?? 1) . ' ' . $invoice->billingUnitLabel();
                            break;
                        default:
                            $rotationType = '';
                    }
...
@push('scripts')
    <script>
        ...
        $('body').on('change keyup', '#rotation, #billing_cycle, #billing_interval, #billing_unit', function(){
            ...
            case 'daily':
                ...
                break;
            case 'weekly':
                rotationType = "{{__('app.every')}}"+' '+"{{__('modules.recurringInvoice.week')}}";
                break;
            case 'bi-weekly':
                rotationType = "{{__('app.every')}}"+' '+"{{__('app.bi-week')}}";
                break;
            case 'monthly':
                rotationType = "{{__('app.every')}}"+' '+"{{__('app.month')}}";
                break;
            case 'quarterly':
                rotationType = "{{__('app.every')}}"+' '+"{{__('app.quarter')}}";
                break;
            case 'half-yearly':
                rotationType = "{{__('app.every')}}"+' '+"{{__('app.half-year')}}";
                break;
            case 'annually':
                rotationType = "{{__('app.every')}}"+' '+"{{__('app.year')}}";
                break;
            case 'custom':
                var interval = $('#billing_interval').val() || 1;
                var unitLabel = $('#billing_unit option:selected').text();
                rotationType = "{{__('app.every')}}" + ' ' + interval + ' ' + unitLabel;
                break;
            default:
            //
            }
        ...
        function toggleCustomIntervalFields() {
            if ($('#rotation').val() === 'custom') {
                $('#custom_interval_fields').removeClass('d-none');
                $('#billing_unit').selectpicker('refresh');
            } else {
                $('#custom_interval_fields').addClass('d-none');
            }
        }
        ...
        switch (rotation) {
            case 'daily':
                rotationDate = nextDate.add(1, 'days');
                break;
            case 'weekly':
                rotationDate = nextDate.add(1, 'weeks');
                break;
            case 'bi-weekly':
                rotationDate = nextDate.add(2, 'weeks');
                break;
            case 'monthly':
                rotationDate = nextDate.add(1, 'months');
                break;
            case 'quarterly':
                rotationDate = nextDate.add(1, 'quarters');
                break;
            case 'half-yearly':
                rotationDate = nextDate.add(2, 'quarters');
                break;
            case 'annually':
                rotationDate = nextDate.add(1, 'years');
                break;
            case 'custom':
                var interval = parseInt($('#billing_interval').val(), 10) || 1;
                var unit = $('#billing_unit').val() || 'months';
                rotationDate = nextDate.add(interval, unit);
                break;
            default:
                rotationDate = nextDate.add(1, 'days');
        }
