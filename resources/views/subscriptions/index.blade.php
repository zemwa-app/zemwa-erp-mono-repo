...
<x-filters.filter-box>
    ...
    <div class="select-box d-flex pr-2 border-right-grey border-right-grey-sm-0">
        ...
        <div class="select-status d-flex">
            <input type="text"
                ...
                id="subscriptionRange" placeholder="@lang('placeholders.dateRange')">
        </div>
    </div>
    ...
    <div class="select-box d-flex py-2 px-lg-2 px-md-2 px-0 border-right-grey border-right-grey-sm-0">
        ...
        <div class="select-status">
            <select class="form-control select-picker" id="filter_client" data-live-search="true" data-size="8">
                <option value="all">@lang('app.all')</option>
...
@push('scripts')
    ...
    <script>
        // ── Date picker ──────────────────────────────────────────
        $('#subscriptionRange').daterangepicker({
        ...
        $('#subscriptionRange').on('apply.daterangepicker', function (ev, picker) {
            $(this).val(
        ...
        $('#subscriptionRange').on('cancel.daterangepicker', function () {
            $(this).val('');
            updateResetBtn();
            showTable();
        });
        ...
        // ── Filter selects ────────────────────────────────────────
        $('#filter_status, #filter_client, #filter_rotation').on('change', function () {
            updateResetBtn();
            showTable();
        });

        $('#reset-filters').on('click', function () {
            $('#filter_status, #filter_client, #filter_rotation').val('all').selectpicker('refresh');
            $('#subscriptionRange').val('');
        ...
        function updateResetBtn() {
            var dirty = $('#filter_status').val()   !== 'all'
                     || $('#filter_client').val()   !== 'all'
                     || $('#filter_rotation').val() !== 'all'
                     || $('#subscriptionRange').val() !== '';
            dirty ? $('#reset-filters').removeClass('d-none') : $('#reset-filters').addClass('d-none');
        }

        // ── DataTable pre-XHR hook (passes filter values to server) ──
        $('#subscriptions-table').on('preXhr.dt', function (e, settings, data) {
            var range = $('#subscriptionRange').val();
            ...
            data['client_id'] = $('#filter_client').val();
            data['rotation']  = $('#filter_rotation').val();
            data['startDate'] = startDate;
            data['endDate']   = endDate;
        });

        const showTable = () => window.LaravelDataTables['subscriptions-table'].draw(true);

        // ── Action handlers ───────────────────────────────────────
        $('body').on('click', '.btn-view-logs', function () {
            var url = $(this).data('url');
            $(MODAL_LG + ' ' + MODAL_HEADING).html('...');
            $.ajaxModal(MODAL_LG, url);
        });

        $('body').on('click', '.btn-reactivate-subscription', function () {
            var id = $(this).data('id');

            Swal.fire({
                title: "@lang('messages.sweetAlertTitle')",
                text: "@lang('messages.subscriptionReactivateText')",
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    $.easyAjax({
                        type: 'POST',
                        url: "{{ route('subscriptions.reactivate', ':id') }}".replace(':id', id),
                        blockUI: true,
