                    ...
                    <x-slot name="thead">
                        ...
                        <th>@lang('modules.accountSettings.currencyFormat')
                            <i class="fa fa-question-circle" data-toggle="tooltip"
                               data-original-title="@lang('modules.accountSettings.currencyFormatSettingTooltip')"></i>
                        </th>
                        ...
                        <tr class="row{{ $currency->id }}">
                            ...
                            <td>
                                <span data-toggle="tooltip"
                                      data-original-title="1 {{global_setting()->currency->currency_code}} = {{$currency->exchange_rate}} {{$currency->currency_code}}">
                                {{ !is_null($currency->exchange_rate) ? $currency->exchange_rate : '--' }}</span>
                            </td>

                            <td> {{ global_currency_format(1000, $currency->id) }}</td>
                            <td class="text-right">
                                <div class="task_view">
                                    <a class="task_view_more d-flex align-items-center justify-content-center edit-channel"
                                       data-currency-id="{{ $currency->id }}" href="javascript:;">
                                ...
                                @if (global_setting()->currency_id != $currency->id)
                                    <div class="task_view mt-1 mt-lg-0 mt-md-0">
                                        <a class="task_view_more d-flex align-items-center justify-content-center delete-table-row"
                                           href="javascript:;" data-currency-id="{{ $currency->id }}">
