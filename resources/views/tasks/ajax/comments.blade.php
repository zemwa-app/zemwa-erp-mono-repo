...
<style>
    ...
    #task-comment-bulk-action-bar .form-check,
    .task-comment-card-checkbox {
        display: flex;
        align-items: center;
        padding-left: 0;
    }

    #task-comment-bulk-action-bar .form-check-input,
    .task-comment-card-checkbox .form-check-input {
        position: static;
        margin: 0 6px 0 0;
    }
...
<div class="tab-pane fade show active" role="tabpanel" aria-labelledby="nav-email-tab">
    ...
    || ($addTaskCommentPermission == 'both' && (in_array(user()->id, $taskUsers) || $task->added_by == user()->id || $task->added_by == $userId))
        ...
        @endif
    @endif
    @if ($deleteTaskCommentPermission == 'all' || $deleteTaskCommentPermission == 'added')
        <div class="p-20 px-20 pb-2 d-flex align-items-center justify-content-between border-top pt-2" id="task-comment-bulk-action-bar">
            <div class="form-check mb-0">
                <input type="checkbox" class="form-check-input" id="select-all-task-comments">
                <label class="form-check-label text-dark-grey" for="select-all-task-comments">@lang('modules.permission.selectAll')</label>
            </div>
            <button type="button" class="btn btn-sm btn-danger d-none" id="delete-selected-task-comments">
                @lang('app.delete') (<span id="selected-task-comment-count">0</span>)
            </button>
        </div>
    @endif
            ...
            <div class="card w-100 rounded-1 border-2 mb-3 p-2 comment">
                <div class="card-horizontal flex-row flex-wrap">
                    @if ($deleteTaskCommentPermission == 'all' || ($deleteTaskCommentPermission == 'added' && ($comment->added_by == user()->id || $comment->added_by == $userId || in_array($comment->added_by, $clientIds))))
                        <div class="form-check task-comment-card-checkbox mb-0 mr-2 mt-2">
                            <input class="form-check-input task-comment-checkbox" type="checkbox" value="{{ $comment->id }}">
                        </div>
                    @endif
            ...
            $.easyAjax({
                ...
                success: function(response) {
                    if (response.status == "success") {
                        ...
                        $('#selected-task-comment-count').text(0);
                        $('#delete-selected-task-comments').addClass('d-none');
