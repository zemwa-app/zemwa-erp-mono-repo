<div class="modal-header">
    <h5 class="modal-title" id="modelHeading">@lang('modules.timeLogs.addMemoAfterAutoStop')</h5>
    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
            aria-hidden="true">×</span></button>
</div>
<div class="modal-body">
    <x-alert type="info">@lang('modules.timeLogs.autoStoppedMemoInfo')</x-alert>
    <x-form id="autoStoppedMemoForm">
        <div class="row">
            ...
            <div class="bootstrap-timepicker timepicker col-sm-6">
                ...
                value="{{ (!is_null($timeLog->end_time)) ? $timeLog->end_time->timezone(company()->timezone)->format(company()->time_format) : '' }}">
            </div>
            <div class="bootstrap-timepicker timepicker col-sm-6">
            ...
            <div class="col-sm-12">
                <x-forms.textarea fieldId="memo" :fieldLabel="__('modules.timeLogs.memo')"
                    fieldName="memo" fieldRequired="true" :fieldValue="$timeLog->memo" fieldPlaceholder="">
                </x-forms.textarea>
            </div>
        </div>
    </x-form>
...
<div class="modal-footer">
    ...
    <x-forms.button-primary id="save-auto-stopped-memo" icon="check">@lang('app.save')</x-forms.button-primary>
</div>

<script>
    $('#save-auto-stopped-memo').click(function() {
        var url = "{{ route('timelogs.save_auto_stopped_memo') }}";
        $.easyAjax({
            url: url,
            container: '#autoStoppedMemoForm',
            type: "POST",
            data: $('#autoStoppedMemoForm').serialize(),
